...
from price_fetcher import average_last_10_closed_sales, best_sell_price_from_api
...
def fetch_selected_relics_from_api_sync(selected_relic_names: list[str]) -> pd.DataFrame:
    ...
    def get_price(slug: str | None) -> float | None:
        if slug is None:
            return None
        ...
        price = best_sell_price_from_api(slug)
    ...
    for i, relic in enumerate(relics, start=1):
        ...
        try:
            ...
            price_a1 = get_price(reward_slug(a1))
            price_a2 = get_price(reward_slug(a2))
            price_g1 = get_price(reward_slug(g1))
